...
/// @title Multicall2 - Aggregate results from multiple read-only function _calls
...
contract Multicall2 {
    ...
    function blockAndAggregate(Call[] calldata _calls) public returns (uint256 blockNumber, bytes32 blockHash, Result[] memory returnData) {
        (blockNumber, blockHash, returnData) = tryBlockAndAggregate(true, _calls);
    }
    ...
    function tryBlockAndAggregate(bool requireSuccess, Call[] calldata _calls) public returns (uint256 blockNumber, bytes32 blockHash, Result[] memory returnData) {
        blockNumber = block.number;
        blockHash = blockhash(block.number);
        returnData = tryAggregate(requireSuccess, _calls);
    }
}
